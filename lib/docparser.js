...
YUI.add('docparser', function(Y) {
    ...
    */
    ...
    readResources = function(target, classname, extNames) {
        ...
        if (!resourcedir) { return target; }
        
        var dirpath = name;
        var resourceList = [];
        
        try { resourceList = fs.readdirSync(resourcedir + '/' + dirpath); } catch(e) { }
        
        resourceList.forEach(function (filename) {
            
            var filePath = resourcedir + '/' + dirpath + '/' + filename; 
            var fileHref = require('path').relative(outdir + '/classes', resourcedir) + '/' + dirpath + '/' + filename;
            
            var fileExt = path.extname(filename);
            var title = unorm.nfc(path.basename(filename, fileExt));
            
            var type = extNames[fileExt];
            if (!type) { return; }
            ...
            switch (type) {
            case 'manual':
                item.description = fs.readFileSync(filePath, 'utf8');
                ...
                case '.markdown':
                    item.description = MD(item.description).replace(/<pre><code>/g, '<pre class="code"><code class="prettyprint">');
                    break;
                }
                
                item.description = item.description.replace(/(<(a|img)\s[^>]*)(href|src)="([^"]+)"/gi, function(_, pre, tag, attr, src) {
                    if (!/^\w+:\/\//.test(src)) { src = path.dirname(fileHref) + '/' + src; }
                    return pre + attr + '="' + src + '"'; 
                });
            ...
            case 'demo':
                ...
                var result = (
                    /<meta\sname=['"]?description['"]?[^>]+content="([^"]*)"/i.test(content) ||
                    /<meta\sname=['"]?description['"]?[^>]+content='([^']*)'/i.test(content) ||
                    /<meta\sname=['"]?description['"]?[^>]+content=([^\s]*)/i.test(content)  ||
                    /<meta\scontent="([^"]*)"[^>]+name=['"]?description['"]?/i.test(content) ||
                    /<meta\scontent='([^']*)'[^>]+name=['"]?description['"]?/i.test(content) ||
                    /<meta\scontent=([^\s]*)[^>]+name=['"]?description['"]?/i.test(content)
                );
                
                result && (item.description = RegExp.$1);
                break;
            */
            }
    ...
    readDemos = function(target, classname) {
        
        var self = this;
        
        var outdir = options.outdir;
        var demodir = options.demodir;
        if (!demodir) { return target; }
        
        console.log('########## READDEMOS : ', classname, options.demodir);
        
        var demoFiles = this.demoFiles;
        if (!demoFiles) {
            ...
            (function(dirpath) {
                ...
                var lists = fs.readdirSync(dirpath);
                for (var i = 0, len = lists.length; i < len; i++) {
                    
                    var filename = lists[i];
                    var fullfile = dirpath + '/' + filename;
                    var stat = fs.statSync(fullfile);
                    
                    if (options.excludes[filename]) {
                        continue;
                    }
                    
                    if (stat.isDirectory()) {
                        arguments.callee(fullfile);
                    }
                    
                    if (!/^[0-9]+\. /.test(filename)) { continue; }
            ...
            for (var i = 0, len = files.length; i < len; i++) {
                ...
                data.metas = parseMetas(data.content);
                data.title = (/<title>(.*?)<\/title>/i.test(data.content) && RegExp.$1) || '';
                demoFiles.push(data);
        ...
        for (var i = 0, len = demoFiles.length; i < len; i++) {
            
            var demoFile = demoFiles[i];
            
            var metas = demoFile.metas;
            var classLists = (metas['class-lists'] || '').split(/\s*,\s*/);
            
            if (classLists.indexOf(classname) === -1) { 
                continue;
            }
            
            var title = demoFile.title;
            var hash = crypto.createHash('md5').update(title).digest('hex');
            
            var item = {
                uid : hash,
                name : title,
                href : path.relative(outdir + '/classes', demoFile.path).replace(/\\/g, '/'),
                // path : filePath,
                //ext : fileExt
                description : metas['description']
            };
            
            target.demos= target.demos || [];
            target.demos.push(item); 
    ...
    parseMetas = function(content) {
        
        var metas = [];
        
        var getAttr = function(key, inner) {
            
            return (
                new RegExp('(^|\\s)' + key + '="(.*?)"', 'i').test(inner) ||
                new RegExp('(^|\\s)' + key + '=\'(.*?)\'', 'i').test(inner) ||
                new RegExp('(^|\\s)' + key + '=([^\\s]+)', 'i').test(inner + ' ')
            ) && RegExp.$2;
            
        };
        
        content.replace(/<meta(\s+[^>]*)?>/gi, function(_, inner) {
            
            var name = getAttr('name', inner);
            if (!name) { return; }
            
            metas[name] = getAttr('content', inner);
...
// REGEX_TYPE = /(.*?)\{(.*?)\}(.*)/,
            ...
            // Extract {type}
            if (match) {
                type = fixType(trim(match[1]));
                desc = trim(match[2]);
            }
            
            /*
            if (value.indexOf('kukuku') > -1) {
                ...
                console.log('optdefault : ', optdefault);
            ...
            */
            ...
            if (/^\[([\w\W]*)\]$/.test(name)) {
                ...
                parts = [ RegExp.$1 ];
                if (RegExp.$3) { parts.push(RegExp.$3); }
                
                if (parts.length > 1) {
                    name = parts[0];
                    optdefault = parts[1].replace(/\\\]/g, ']');
        ...
        'return': function(tagname, value, target, block) {
            ...
            var desc = implodeString(trim(value)), name = '', type,
                match = REGEX_TYPE.exec(desc),
                result = {};
            ...
            // parse .prop, indicating a child property for object
            if (name.indexOf('.') > -1) {
                
                var root = target[tagname];
                var props = root.props = root.props || [];
                
                match = name.split('.');
                for (var i = 0, len = match.length - 2; i < len; i++) {
                    root = props[props.length - 1];
                    props = root.props = root.props || [];
                }
                
                result.name = match[match.length - 1];
                props.push(result);
        ...
        'class': function(tagname, value, target, block) {
            ...
            target = readResources(target, fullname, {
                '.jpg' : 'screenshot',
                '.jpeg' : 'screenshot',
                '.png' : 'screenshot',
                '.gif' : 'screenshot',
            ...
            target = readDemos(target, fullname);
        
            /*
            target = readResources(target, fullname, 'screenshot', [ '.jpg', '.jpeg', '.png', '.gif' ]); 
            target = readResources(target, fullname, 'demo', [ '.html', '.htm' ]); 
            target = readResources(target, fullname, 'manual', [ '.wiki', '.html', '.htm', '.doc', '.docx', '.markdown' ]);
            */
        ...
        'property': function(tagname, value, target, block) {
            ...
            target.itemtype = tagname;
            target.name = tagname === 'constructor' ? this.get(CURRENT_CLASS) : value;
            if (!target.type) {
                desc = implodeString(trim(value)),
                match = REGEX_TYPE.exec(desc);
        ...
        'remark': function(tagname, value, target, block) {
            
            var list = target[tagname] = target[tagname] || [];
            list.push(value.trim());

        },
        
        'see': function(tagname, value, target, block) {
            ...
            var see = {
                href : RegExp.$1,
                description : RegExp.$2 || RegExp.$1,
                target : "blank"
            };
            
            if (!/^[a-z]+:\/\//.test(see.href)) {
                see.href = /#/.test(see.href) ? see.href.replace(/#/, '.html#') : see.href + '.html';
                see.target = "_self";
            }
        ...
        'depends' : function(tagname, value, target, block) {
            if (!Lang.isArray(target[tagname])) {
                target[tagname] = [];
            }
            
            var relation = target['relation'] = target['relation'] || {};
            
            //If the item is @tag one,two
            var list = value.split(/\s*,\s*/);
            relation[tagname] = relation[tagname] || [];
            relation[tagname] = relation[tagname].concat(list);
            
            target[tagname] = target[tagname].concat(list);
        },

        'mixin': 'depends',

        'throws': function(tagname, value, target, block) {
        ...
        'uses': function(tagname, value, target, block) {
            if (!Lang.isArray(target[tagname])) {
                target[tagname] = [];
            }

            var relation = target['relation'] = target['relation'] || {};
            var isGroup = /{([^}]+)}$/.test(value);

            if (isGroup) {
                
                var part = RegExp.$1.split(',');
                
                var group = value.replace(/{([^}]+)}$/, '').trim().split(/\s*,\s*/);

                var from = Number(part[0]) || 0;
                var to = Number(part[1]) || group.length;
                
                part.length === 1 && (to = from);
                target[tagname] = target[tagname].concat(group);
                
                relation[tagname] = relation[tagname] || [];
                relation[tagname].push({
                    from : from,
                    to : to,
                    group : group
                });
        ...
        'refer': function(tagname, value, target, block) {
            if (!Lang.isArray(target[tagname])) {
                target[tagname] = [];
            }
        ...
        'keyword': function(tagname, value, target, block) {
            
            var keywords = value.split(/\s*,\s*/);
            for (var i = 0, len = keywords.length; i < len; i++) {
                
                var keyword = keywords[i].trim();
                var keyworditem = this.keywords[keyword] = this.keywords[keyword] || { 'name' : keyword, 'tag' : 'keyword' };
                var classes = keyworditem.classes = keyworditem.classes || {};
                
                block.forEach(function(i, k) {
                    if (i.tag === 'class') { classes[trim(i.value)] = 1; }
                });
                
            }
            
            return DIGESTERS['todo'].apply(this, arguments);
            
        },

        'history': function (tagname, value, target, block) {
            
            if (!Lang.isArray(target[tagname])) { target[tagname] = []; }
            
            if (/^([^\s]+)\s+([^\s]+)\s+([\w\W]*)$/.test(value)) {
                ...
                var clazz = this.get(CURRENT_CLASS);
                if (clazz) {
                    
                    var clazzObj = this.classes[clazz];
                    var histories = clazzObj.histories = clazzObj.histories || [];
                    
                    var targetStr = target.name; 
        ...
        'factory': function (tagname, value, target, block) {
            target[tagname] = value.toUpperCase() !== 'FALSE';
        },
        
        'syntax': function (tagname, value, target, block) {
            if (!Lang.isArray(target[tagname])) {
                target[tagname] = [];
            }
            target[tagname].push(value.trim().split(/\s*,\s*/).join(', '));
        },
...
// reconsitute and tokenize the comment block
            ...
            var filterTags = (/(?:^|\n)\s*@filter\s+([^\n]+)/.test(comment) && RegExp.$1 || '').trim();
            filterTags = filterTags ? filterTags.split(/\s*,\s*/) : []; 
            
            var filterOption = options.filter;
            
            if (filterTags.length > 0 && filterOption) { // @filter 태그도 있고 filterOption 도 지정되어 있을때
                if (filterTags.indexOf(filterOption) === -1) { // filterOption 으로 지정한게 @filter 에 없으면
                    return null; // 커멘트 처리 안함                   
                }
            }
            ...
            Y.each(filemap, function(code, filename) {
                ...
                for (i = 0; i < len; i++) {
                    ...
                    if(REGEX_START_COMMENT.test(line)) {
                        ...
                        var commentresult = this.handlecomment(comment, filename, linenum); 
                        commentresult && commentmap[filename].push(commentresult);
                    }
                }
            }, this);
...
Y.log('unknown tag: ' + name + ',' + stringlog(block), 'warn', 'docparser');
        ...
        transform: function(commentmap) {
            ...
            Y.each(classes, function(clazz, name) {
                ...
                if (clazz.mixin && clazz.mixin.length) {
                    clazz.mixin.forEach(function(u) {
                        var c = classes[u];
                        if (c) {
