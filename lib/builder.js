...
YUI.add('doc-builder', function(Y) {
    ...
    Y.DocBuilder.prototype = {
        ...
        */
        ...
        /**
        * Populate the meta data for tags
        * @method populateTags
        * @param {Object} opts The original options
        * @return {Object} The modified options
        */
        populateTags: function(opts) {
            opts.meta.tags = [];
            Y.each(this.data.tags, function(v) {
                if (v.external) { return }
                opts.meta.tags.push({ displayName: v.name, name: v.name});
            });
            opts.meta.tags.sort(this.nameSort);
            return opts;
        },
        /**
        ...
        */
        makeDirs: function(cb) {
            var self = this;
            var dirs = ['classes', 'modules', 'files', 'tags'];
        ...
        * @param {Object} cv.view The View Data
        */
        renderTag: function(cb, data, layout) {
            var self = this;
            var stack = new Y.Parallel();
            ...
            Y.prepare([DEFAULT_THEME, themeDir], self.getProjectMeta(), function(err, opts) {
                ...
                opts.meta.tagName = data.displayName || data.name;
                //opts.meta.tagDescription = self._parseCode(self.markdown(data.description || ' '));
                ...
                if (data.classes && Object.keys(data.classes).length) {
                    opts.meta.tagClasses = [];
                    Y.each(Object.keys(data.classes), function(name) {
                        var i = self.data.classes[name];
                        if (i) {
                            opts.meta.tagClasses.push({ name: i.name, displayName: i.name });
                        }
                    });
                    opts.meta.tagClasses.sort(self.nameSort);
                }

                var view   = new Y.DocView(opts.meta);
                var mainLayout = opts.layouts[layout];
                self.render(TMPL_INCLUDE('tag'), view, mainLayout, opts.partials, stack.add(function(err, html) {
                    self.files++;
                    stack.html = html;
                    stack.view = view;
                }));
        ...
        /**
        * Generates the tag files under "out"/tags/
        * @method writeTags
        * @param {Callback} cb The callback to execute after it's completed
        */
        writeTags: function(cb, layout) {
            layout = layout || 'main';
            var self = this,
                stack = new Y.Parallel();
            stack.html = [];
            ...
            var counter = 0;
                Object.keys(self.data.tags).forEach(function(k) {
                    if (!self.data.tags[k].external) {
                        counter++;
                    }
                });
            Y.log('Rendering and writing ' + counter + ' tag pages.', 'info', 'builder');
            
            Y.each(self.data.tags, function(v) {
                if (v.external) { return }
                self.renderTag(function(html, view) {
                    stack.html.push(html);
                    stack.view.push(view);
                    if (self.options.dumpview) {
                        Y.Files.writeFile(path.join(self.options.outdir, 'json', 'tag_' + v.name + '.json'), JSON.stringify(view), stack.add(noop));
                    }
                    Y.Files.writeFile(path.join(self.options.outdir, 'tags', v.name + '.html'), html, stack.add(noop));
                }, v, layout);
            });
            stack.done(function() {
                Y.log('Finished writing tag files', 'info', 'builder');
                cb(stack.html, stack.view);
            });
        },
        ...
        */
        renderClass: function(cb, data, layout) {
            ...
            Y.prepare([DEFAULT_THEME, themeDir], self.getProjectMeta(), function(err, opts) {
                ...
                classItems.forEach(function(i) {
                    ...
                    switch (i.itemtype) {
                        ...
                        case 'method':
                            ...
                            i[i.itemtype + 'Description'] = self._parseCode(i.description);
                ...
                opts.meta = self.readResources(opts.meta, 'screenshot', [ '.jpg', '.jpeg', '.png', '.gif' ]); 
                opts.meta = self.readResources(opts.meta, 'demo', [ '.html', '.htm' ]); 
                opts.meta = self.readResources(opts.meta, 'manual', [ '.wiki', '.html', '.htm', '.doc', '.docx' ]); 
        ...
        */
        ...
        readResources : function(meta, type, extNames) {
            ...
            resourceList.forEach(function (filename) {
                ...
                var description = path.basename(filename, fileExt);
        ...
        */
        renderAPIMeta: function(cb) {
            ...
            ['classes', 'modules', 'tags'].forEach(function(id) {
        ...
        populateSearchData : function(opts) {
            ...
            for (var klassName in data.classes) {
                ...
                (klass.tag || []).forEach(function(tag) {
                    var list = tags[tag] = tags[tag] || [];
                    list.push(klassName); 
                });
            ...
            for (tagname in tags) {
                var list = detail.tags;
                list.push({ name : tagname, class : tags[tagname] });  
            }
            ...
            detail.tags = fpSort(detail.tags);
                ...
                self.makeDirs(function() {
                    ...
                    Y.Files.copyAssets([path.join(DEFAULT_THEME, 'assets'), path.join(themeDir, 'assets')], path.join(self.options.outdir, 'assets'), false, function() {
                        ...
                        */
                        ...
                        self.writeTags(cstack.add(noop));
                        if (!self.options.nocode) {
                            self.writeFiles(cstack.add(noop));
                        }
