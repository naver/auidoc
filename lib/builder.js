...
YUI.add('doc-builder', function(Y) {
    ...
    */
    ...
    // handlebars 와 microtemplate 둘다 쓸 수 있도록
    var templateEngines = {
        'handlebars' : {
            engine : Y.Handlebars,
    ...
    Y.DocBuilder.prototype = {
        ...
        */
        _parseCrossLink: function(item, raw) {
            ...
            } else if (typeDesc = this.options.types[baseItem]) { // 타입설명이 있는 경우
        ...
        */
        populateClasses: function(opts) {
            ...
            Y.each(this.data.classes, function(v) {
                ...
                // 클래스의 설명과 스크린샷, 상속정보, 기타등등 정보도 템플릿에서 얻을 수 있게 객체에 추가
        ...
        */
        mergeExtends: function(info, classItems) {
            var self = this;
            // 기존 YUIDoc 의 @uses 을 @mixin 으로 이름 변경
        ...
        */
        renderClass: function(cb, data, layout) {
            ...
            Y.prepare([DEFAULT_THEME, themeDir], self.getProjectMeta(), function(err, opts) {
                ...
                // 생성자가 여러개 있을 수 있기 때문에 하나만 있다고 간주하고 진행되는 이 코드는 필요없음
                /*
                ...
                classItems.forEach(function(i) {
                    ...
                    switch (i.itemtype) {
                        ...
                        case 'method':
                            ...
                            // @syntax 태그가 정의되어 있으면 그걸 활용
                            if (i.syntax instanceof Array) {
                                
                                // i.hasParams = true;
                                i.paramsList = i.syntax;
                            
                            // @syntax 태그가 없으면 @param 들을 가지고 활용
                            } else {
        ...
        */
        renderAPIMeta: function(cb) {
            ...
            opts = this.populateKeywords(opts);
        ...
        /**
        * 선택다운로더에서 활용할 의존성 데이터 만들기
        * Write the Dependency meta data used for the Selection Downloader
