...
YUI.add('doc-builder', function(Y) {
	...
	var TMPL = Y.MicroTemplate; // Handlebars;
    ...
    TMPL.registerHelper('buildFileTree', function(items, fn) {
        return print(items);
    });
    ...
    Y.DocBuilder = function(options, data) {
        ...
        TMPL.registerHelper('crossLink', function(item, fn) {
            var str = '';
            if (!item) {
                item = '';
            }
        ...
        TMPL.registerHelper('crossLinkModule', function(item, fn) {
            var str = item;
        ...
        TMPL.registerHelper('crossLinkRaw', function(item, fn) {
            var str = '';
            if (!item) {
                item = '';
            }
        ...
        _addHelpers: function(helpers) {
            ...
            helpers.forEach(function(imp) {
                ...
                Object.keys(h).forEach(function(name) {
                    TMPL.registerHelper(name, h[name]);
                });
            });
        },
        /**
        ...
        */
        render: function(source, view, layout, partials, callback) {
            ...
            Y.each(parts, function(source, name) {
                TMPL.registerPartial(name, source);
            });

            if (!TEMPLATE || !this.cacheTemplates) {
                TEMPLATE = TMPL.compile(layout);
            }
